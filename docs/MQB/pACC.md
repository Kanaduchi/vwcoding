...
1. Установка X прошивки. Например, для радара, который указан выше - FL_2Q0907572R_X371___S.odx

2. После прошивки включится защита компонентов на радар.  
Защиту компонентов можно через ODIS Online снять, либо у местного дилера VW, либо у каких-нибудь контор, которые занимаются активацией, свапами и т.д.

3. Для генерирования SWaP кодов понадобится VCRN (Vehicle Component Registration Number).  
...
5. С помощью утилиты afcg.exe сгенерировать SWAP код. Для генерации кода понадобится ввести: VIN, VCRN (из 3 шага) и набор FEC кодов через пробел     

6. Полученный SWAP код необходимо ввести в адаптации 13 блока (Передача кода разблокировки функции SWAP)  
   
7. Установка последней прошивки. В случае с приведенным блоком - FL_2Q0907572T_0383___S.odx  

8. Заливка нужной параметрии DA_013_7202_550_T38O_FF_VW376.xml  

9. Проведение кодировок и адаптаций:
