...
``` yaml
    Block 08 → Coding:
    Byte 11 – Bit 6 (Blower display during auto mode): Activate 
...
Fan mode indication in automatic mode: activated
    ```
...
If there is no 6th bit, you can only change the code itself - replace the value 00001110 with value 01001110
    Exit → Save  
    ```
...
``` yaml
    ...
    → Apply
    Speicherung der Sitzheizungsstufe Beifahrer activate (Retention of passenger’s seat heater level): Activate
    → Apply
    ```
...
Driver seat heating level retention: act
Front passenger seat heating level retention: act
    ```
...
- Steering wheel heating, automatic mode: “not set” to the desired value
    → Apply
    ```
...
Byte 13 – Bit 2: according to the sensor in the steering wheel
Byte 13 – Bit 3: according to outside temperature
Both bits cannot be turned on at the same time!
If there is no choice of bits, then you can change the value of the byte itself: 14 or 18
    Exit → Save 
    ```
...
``` yaml title="Login code: 31347"
    Block 09 → Coding:
    Mirror heating on while rear window heater is on: activate
    → Apply (with block reboot)
    ```
...
```yaml title="Login code: 31347"
    09 – On-board network electronics  
    Coding - 07 → Long coding:
...
``` yaml
    ...
    - Comfort systems consumption, exterior mirror heating dependency → Independent of outside temperature without switch  
    Range increase:  
    - Mirror heating consumption when rear window heater is on: Activate
    ```
...
Block 09 → Adaptation:
Window heating:
- Frontscheibenheizung Zeitwert: “320 s” to “640 s”
- Abschalttemperatur fuer Frontscheibenheizung: «35.0 °C» to «38.0 °C»
→ Apply
```
