...
3. Должны увидеть:  
    ...
    FINISHED — You can now remove SD Card
    ```
4. Иногда скрипт не выполняется, и выдает ошибку. Тогда шаги из скрипта необходимо выполнить вручную:  
    ```
    cd / && mount -uw /sdc1/
    umount -f /extbin
    ...
    ```
5. Перезагружаем магнитолу долгим нажатием на кнопку включения — ОБЯЗАТЕЛЬНО!!!!
6. Монтируем SD карту  
    ```
    cd / && mount -uw /sdc1/
    ```
7. Запускаем скрипт Патч FEC кодов [(MST2_fec.sh)](../firmwares/MST2_fec.sh)  
    ```
    cd / && /sdc1/MST2_fec.sh
    ```
8. Должны увидеть:  
    ...
    *.FEC installation — ok
    ...
    FINISHED — You can now remove SD Card
    ```
9. Перезагружаем магнитолу долгим нажатием на кнопку включения — ОБЯЗАТЕЛЬНО!!!!  
